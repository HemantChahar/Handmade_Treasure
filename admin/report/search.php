		...
		<div class="panel-body">
			<div class="row">
				<form class="form-inline pull-right" action="index.php" method="post">
					...
					<div class="form-group">
						<input class="form-control date start input-lg" size="20" type="date"
							value="<?php echo (isset($_POST['start'])) ? $_POST['start'] : ''; ?>"
							Placeholder="Check In" name="start" id="from" data-date="" data-date-format="yyyy-mm-dd"
							data-link-field="any" data-link-format="yyyy-mm-dd">
					</div>
					<div class="form-group">
						<input class="form-control date end input-lg" size="20" type="date"
							value="<?php echo (isset($_POST['end'])) ? $_POST['end'] : ''; ?>" name="end" id="end"
							data-date="" data-date-format="yyyy-mm-dd" data-link-field="any"
							data-link-format="yyyy-mm-dd">
					</div>

					<button type="submit" name="submit" class="btn btn_kcctc btn-lg"><span
			...
			<form method="post" action="<?php echo $_SERVER['PHP_SELF'] ?>">
				<span id="printout">
					<div class="row">
						<h3 align="center">Overall Income in
							<?php echo ((@$_POST['start']) ? $_POST['start'] : '') . ' to ' . ((@$_POST['end']) ? $_POST['end'] : ''); ?>
						</h3>
					</div>
...
// AND c.`CUSTOMERID` = s.`CUSTOMERID` 
				...
				FROM  `tblcustomer` c,  `tblsummary` s WHERE  c.`CUSTOMERID` = s.`CUSTOMERID` AND  ORDEREDSTATS='Confirmed' AND date(ORDEREDDATE)>='" . $_POST['start'] . "' AND date(ORDEREDDATE) <='" . $_POST['end'] . "'";
					...
					<table>
						<tfoot style="margin-right:10%">
							<tr>
								<h4 align="right"> Overall Price : &#8377 <?php echo isset($overall) ? $overall : 0; ?>
								</h4>
							</tr>
				...
				<button onclick="tablePrint();" class="btn btn_fixnmix"><span class="glyphicon glyphicon-print"></span>
					Print Report</button>
